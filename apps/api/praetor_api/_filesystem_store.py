"""Workspace and state-directory filesystem helpers extracted from storage.py.

The bulk of FilesystemStore writes Markdown into the workspace - mission
folders, wiki pages, matter briefs, decision logs - while a small set of
legacy JSON paths on state_dir feeds the one-time migration into SQLite.

Keeping this in its own module so storage.py can stay focused on the
SQLite index + the AppStorage facade.
"""
...
class FilesystemStore:
    ...
    def append_matter_decision_workspace_file(self, workspace_root: Path, decision: MatterDecisionRecord) -> None:
    ...
    def append_open_question_workspace_file(self, workspace_root: Path, question: OpenQuestionRecord) -> None:
